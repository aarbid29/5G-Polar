...
class PolarDecDataset(Dataset):
    ...
    def __init__(self, snr_db,  num_samples, snr_noise_std=0.1, fixed_msg_bit_size = None, transform = None):
        super().__init__()
    ...
    def __getitem__(self, idx):
         ...
         channel_observation_vector, frozen_bit_prior, target = generate_data(message_bit_size=np.random.choice([8, 16, 24]) if self.fixed_msg_bit_size is None else self.fixed_msg_bit_size, SNRs_db=[self.snr_db])
